...
void StudentTests() {
    ...
    // =======================================================
    Table<int> t(5, 5, 150);
    assert (t.get(0, 0) == 150);
    assert (t.get(1, 4) == 150);
    assert (t.get(1, 2) == 150);
    assert (t.numRows() == 5);
    assert (t.numColumns() == 5);
    // t.print();
    // std::cout << "constructor test completed" << std::endl;
    ...
    // t.set(200, 3, 4); // invalid out of bound request
    // another.set(100, 2, 3); // invalid out of bound request

    Table <std::string> omg(7, 7, "DATAS");
    assert (omg.get(0, 0) == "DATAS");
    assert (omg.get(1, 4) == "DATAS");
    assert (omg.get(1, 2) == "DATAS");
    assert (omg.numRows() == 7);
    assert (omg.numColumns() == 7);
    omg.print();

    std::vector <std::string> column_temp_vector = {"Jesus", "Jesus", "Jesus", "Jesus", "Jesus", "Jesus", "Jesus"};
    std::vector <std::string> row_vector = {"CRiST", "CRiST", "CRiST", "CRiST", "CRiST", "CRiST", "CRiST", "CRisT"};
    omg.push_back_row(column_temp_vector);
    omg.print();
    omg.push_back_column(row_vector);
    omg.print();
